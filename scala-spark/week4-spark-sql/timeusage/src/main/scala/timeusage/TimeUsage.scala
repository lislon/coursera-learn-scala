...
//    val finalDf = timeUsageGroupedSql(summaryDf)
  ...
  def row(line: List[String]): Row =
    Row.merge( Row(line.head) :: line.tail.map(x => Row(x.toDouble)) : _* )
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def is(colName: String, filterBy: String*)  = filterBy.exists(colName.startsWith)

    val groups = columnNames.groupBy(name =>
      if (is(name, "t01", "t03", "t11", "t1801", "t1803")) 1
      else if (is(name, "t05", "t1805")) 2
      else if (is(name, "t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")) 3
      else  4
    ).mapValues(_.map(col))
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column =
      when($"tesex" === "1", "male").otherwise("female")
      .as("sex")
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce((a1, a2) => a1 + a2) / 60).as("primaryNeeds")
    val workProjection: Column = (workColumns.reduce((a1, a2) => a1 + a2) / 60).as("work")
    val otherProjection: Column = (otherColumns.reduce((a1, a2) => a1 + a2) / 60).as("other")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .orderBy($"key._1", $"key._2", $"key._3")
      .map(v => TimeUsageRow(v._1._1, v._1._2, v._1._3,
        v._2, v._3, v._4)
      )
  }
}
