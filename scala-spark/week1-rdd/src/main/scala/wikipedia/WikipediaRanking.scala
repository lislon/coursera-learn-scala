...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("scala-app")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd
      .map(a => if (a.mentionsLanguage(lang)) 1 else 0)
      .aggregate(0)((acc, article) => acc + article, Integer.sum)
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(a => langs.filter(a.mentionsLanguage).map((_, 1)))
      .reduceByKey(_ + _)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.mapValues(v => v.size).sortBy(_._2, ascending = false).collect.toList
