...
class HuffmanSuite {
  ...
  @Test def `effective Encode`: Unit = {
    val tree = createCodeTree("abcaabdddddddddddddddddd".toCharArray.toList)
    assertEquals(List(Fork(Leaf('e',1),Leaf('t',2),List('e', 't'),3), Leaf('x',4)), tree)
  }

  @Test def `decode and encode a very short text should be identity (10pts)`: Unit =
    new TestTrees {
      private val text = "lisa"
      private val bits: List[Bit] = encode(Huffman.frenchCode)(text.toList)
      assertEquals(text.toList, decode(Huffman.frenchCode, bits))
    }
